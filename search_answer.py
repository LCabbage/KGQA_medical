...
class AnswerSearching:
    def __init__(self):
        self.graph = Graph("http://localhost:7474", username="neo4j", password="147258")
        self.top_num = 10
    ...
    def question_parser_graph(self, data):  # 输入的是从问题中抽取的到的实体
        ...
        if data:
            if data.get("Disease"):
                sql,tag = self.transfor_to_sql_graph("Disease", data["Disease"], data["Intentions"])
            elif data.get("Alias"):
                sql,tag = self.transfor_to_sql_graph("Alias", data["Alias"], data["Intentions"])
            elif data.get("Symptom"):
                sql,tag = self.transfor_to_sql_graph("Symptom", data["Symptom"], data["Intentions"])
            elif data.get("Complication"):
                sql,tag = self.transfor_to_sql_graph("Complication", data["Complication"], data["Intentions"])
    ...
    def transfor_to_sql_graph(self, label, entities, intent):
        ...
        if label == "Disease":
            for e in entities:
                sql = ["match (d:Disease{name:'%s'})<-[r]-(p) return r Union all match (d:Disease{name:'%s'})-[r]->(n) return r" %(e,e)]
                sqls+=sql
                tag = "Disease"
        elif label == "Alias":
            for e in entities:
                sql = ["match (d:Disease)-[:ALIAS_IS]->(a:Alias{name:'%s'}) with d (match (p)-[r]->(d:Disease) return r Union all match (d:Disease)-[r]->(n) return r)" % e]
                sqls += sql
                tag = "Disease"
        elif label=="Symptom":
            for e in entities:
                sql = ["match (p)-[r]->(s:Symptom{name:'%s'}) return r Union all match (s:Symptom{name:'%s'})-[r]->(n) return r" % (e,e)]
                sqls += sql
                tag = "HAS_SYMPTOM"
        elif label=="Complication":
            for e in entities:
                sql = ["match (p)-[r]->(c:Complication{name:'%s'}) return r Union all match (p:Complication{name:'%s'})-[r]->(n) return r" % (e,e)]
                sqls += sql
                tag = "Disease"
